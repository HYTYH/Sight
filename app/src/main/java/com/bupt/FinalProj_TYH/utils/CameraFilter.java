...
/*
 * Class:   CameraFilter
 * 职能:    所有filters的基类，提供与filters具体实现类的接口以及相关方法，对OpenGL库中相关方法进行调用
 */
public abstract class CameraFilter {
    ...
    private static MyGLBuffer CAMERA_RENDER_BUF;
    ...
    public CameraFilter(Context context) {
        ...
        if (PROGRAM == 0) {
            PROGRAM = MyOpenGL.buildProgram(context, R.raw.vertext, R.raw.original_rtt);
        }
    }
    ...
    final public void draw(int cameraTexId, int canvasWidth, int canvasHeight) {
        ...
        if (CAMERA_RENDER_BUF == null ||
            ...
            CAMERA_RENDER_BUF = new MyGLBuffer(canvasWidth, canvasHeight, BUF_ACTIVE_TEX_UNIT);
        }
    ...
    public void setupShaderInputs(int program, int[] iResolution, int[] iChannels, int[][] iChannelResolutions) {
        setupShaderInputs(program, VERTEX_BUF, TEXTURE_COORD_BUF, iResolution, iChannels, iChannelResolutions);
    }

    public void setupShaderInputs(int program, FloatBuffer vertex, FloatBuffer textureCoord, int[] iResolution, int[] iChannels, int[][] iChannelResolutions) {
        GLES20.glUseProgram(program);
