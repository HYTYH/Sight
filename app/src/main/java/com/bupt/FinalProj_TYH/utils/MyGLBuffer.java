...
/*
 * Class:   MyGLBuffer
 * 职能:    对帧缓冲区进行相关设置，bind方法将内容显示在全屏，类中实现绑定一个命名的帧缓冲区对象，将纹理图像附加到帧缓冲对象，最后将将渲染缓冲区挂载到当前帧缓冲区上。
 *          被JFAVoronoiFilter（泰勒多边形）调用
 */
public class MyGLBuffer {
    private int texId = 0;
    ...
    public MyGLBuffer(int width, int height, int activeTexUnit) {
        this.width = width;
        this.height = height;
